...
export class ContactComponent implements OnInit {
  ...
  ngOnInit() {
    this._client.getAllMessages().subscribe(data => {
      this._messages = data;
  ...
  openMessage(message: MessageReadModel) {
    const dialogRef = this._dialog.open(MessageModalComponent, {
      disableClose: true,
      data: message
    });
  ...
  private refreshDataSource(): void {
    this.dataSource.data = this._messages.sort(descendingDateSorter(x => x.timestamp));
  }

  private markAsRead(message: MessageReadModel): void {
    if (!message.isRead) {
      this.updateTableMessageAsRead(message);
      this._client.markAsRead(message.id).subscribe(
        () => null,
        err => {
